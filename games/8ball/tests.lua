...
function Tests.impulse_velocity()
    ...
    -- After 1s with damping 0.7: v ≈ v0 * e^(-0.7) ≈ 2817 * 0.497 ≈ 1400
    -- But Box2D damping formula differs slightly; just check reasonable range
    assert_true("velocity_after_1s_decayed", vx < 1500 and vx > 100)

    -- v^2 should be near damping threshold
    local v2 = vx * vx
    assert_true("v2_near_damping_threshold", v2 < 200000)
...
function Tests.damping_system()
    ...
    -- Set velocity to just below damping threshold (v^2 = 90000 -> v = 300)
    wb.body:setLinearVelocity(200, 0)

    -- Simulate damping check (need to call enough times to pass the 10-frame throttle)
    for i = 1, 15 do
        Physics.applyCustomDamping(phys, 2.0)
    end

    -- Damping should have been increased
    local damping = wb.body:getLinearDamping()
    -- v=200, v^2=40000 < 90000 -> increased damping should be 0.7
    -- v^2=40000 > 22500 -> not double damping yet
    assert_eq("damping_increased_to_0.7", damping, 0.7, 0.01)

    -- Set velocity below double threshold (v^2 = 22500 -> v = 150)
    wb.body:setLinearVelocity(100, 0)
    wb.state = Config.BALL_STATE_RUN
    wb.body:setLinearDamping(0.7) -- reset

    for i = 1, 15 do
        Physics.applyCustomDamping(phys, 2.0)
    end

    -- v=100, v^2=10000 < 22500 -> double damping = 1.0
    -- But per-ball stop: v=100 > BALL_VELOCITY_LIMIT(4), so ball should still be running
    -- Actually v=100 >> 4, so it won't be stopped
    damping = wb.body:getLinearDamping()
    assert_eq("damping_doubled_to_1.0", damping, 1.0, 0.01)

    -- Per-ball individual stop: set velocity below BALL_VELOCITY_LIMIT
    wb.body:setLinearVelocity(3, 2)
    wb.state = Config.BALL_STATE_RUN
    for i = 1, 15 do
        Physics.applyCustomDamping(phys, 2.0)
    end
    -- |vx|=3 < 4, |vy|=2 < 4 -> ball should be stopped
...
function Tests.physics_simulation()
    ...
    -- Some colored balls should have moved
