...
function Physics.createBall(self, index, dx, dy)
    ...
    -- Mass matches original: density(2.7) * pi * r^2 = ~1908.5
    -- No mass override needed now; impulse is scaled by CUE_DISTANCE in cue.lua
    ...
    self.balls[index] = {
        ...
        rollX = math.random() * math.pi * 2,  -- rotation around X axis (from vy)
        rollY = math.random() * math.pi * 2,  -- rotation around Y axis (from vx)
        rollZ = math.random() * math.pi * 2,  -- rotation around Z axis (spin/english)
    }
...
-- Update 3D rolling angles from linear velocity.
-- Real physics: a ball rolling without slipping has angular_velocity = linear_velocity / radius.
-- The rotation axis is perpendicular to the velocity direction.
--   - Moving right (vx > 0) -> ball rolls forward around X axis (top goes away)
--   - Moving up (vy < 0 in screen) -> ball rolls around Y axis
-- Additionally, Box2D angular velocity drives Z-axis spin (top spin / english).
function Physics.updateBallRolling(self, dt)
    ...
    for _, ball in pairs(self.balls) do
        if ball.state ~= Config.BALL_STATE_IN_HOLE and ball.visible then
            ...
            if speed2 > 1 then
                -- Pure rolling: angular_vel = velocity / radius
                -- X-axis rotation: driven by forward/backward movement (vy)
                -- Y-axis rotation: driven by left/right movement (vx)
                -- The rotation is around the axis PERPENDICULAR to movement.
                -- Moving right (vx+) rotates the ball forward around a horizontal axis
                -- perpendicular to the direction of travel.
                --
                -- For the shader (which applies rotations as Euler Z->Y->X):
                --   rollX controls tilt up/down (pitch) - driven by vy
                --   rollY controls tilt left/right (yaw) - driven by vx
            ...
            -- Z-axis rotation: spin from Box2D angular velocity (english/masse)
